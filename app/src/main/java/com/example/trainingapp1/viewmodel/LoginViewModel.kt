...
import com.example.trainingapp1.network.data.RetrofitInstance
    ...
    fun login(email: String, password: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                val loginService = RetrofitInstance.getLoginService()
                val responseService = loginService.getLogin(LoginDto(email = email, password = password))
